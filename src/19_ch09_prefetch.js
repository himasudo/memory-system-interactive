/* ======================= chapter: prefetchers ======================= */
App.chapter({id: 'pref', num: '09', short: 'Prefetchers', title: 'Prefetchers: fetching the next line before it is asked for',
sub: 'A prefetcher watches the addresses a core misses on and bets on the next ones. When the bet is right a DRAM latency disappears; when the pattern has no structure, nothing can be predicted.',
build: function(root){
  var h = App.h, s = App.s, g = App.g, CFG = App.CFG;
  var pat = 'seq', dist = 4, onPF = true;
  var intro = h('div', {'class': 'grid2'}, root);
  h('div', {'class': 'card'}, intro, '<h3>What is modelled</h3><p>One detector watches the sequence of cache lines a load touches. When the distance between consecutive lines repeats (+1 for a ' + g('stream', 'stream') + ', +k for a ' + g('stride') + '), it becomes confident and requests the next <i>distance</i> lines ahead. A prefetch that arrives before the load hits; one still in flight is ' + g('late') + '; lines prefetched but never used are ' + g('pollute', 'pollution') + '. Latency per line = the DRAM value in the latency settings (' + App.dramCycles() + ' cycles now).</p>');
  h('div', {'class': 'card'}, intro, '<h3>What is not published</h3><p>Zen+ has hardware prefetchers at the L1d and at the L2. AMD does not publish their table sizes, confidence thresholds or prefetch distances. The detector here uses one tracking entry, confidence after two matching deltas, and the distance you choose. Use it to reason about which access patterns can be predicted at all, not to predict exact numbers.</p>');
  ...
  function draw(){
    ...
    var txt = {
      seq: 'The histogram reads <code>data[]</code> one byte per iteration, so it enters a new line every ~100 cycles (model). After two +1 steps the detector is confident. With distance ' + dist + ', prefetches are issued ' + dist + ' lines (' + dist * 100 + ' cycles) ahead against a ' + st.D + '-cycle latency: ' + (dist * 100 >= st.D ? 'far enough, so accesses after warm-up are covered.' : 'not far enough, so prefetches are late and part of each latency remains.') + ' Sequential access is why earlier benchmarks in this course never saw DRAM latency directly.',
      ...
      chase: 'Each address is the data returned by the previous load, and the next line is random. The detector sees no repeating delta, and even a perfect predictor could not help because the address does not exist until the previous miss returns. Every access pays the full latency, one after another: this is the pattern Block C uses to measure true latency.'
    }[pat];
