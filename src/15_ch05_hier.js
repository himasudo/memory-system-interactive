...
build: function(root){
  ...
  h('div', {'class': 'card'}, intro, '<h3>Same address, bigger caches</h3><p>The line is PA <code>' + hx(LINE) + '</code>. The L2 (512 KB, 8-way) has 1024 sets; a straightforward PA[15:6] indexing model gives <b>set ' + l2set + '</b>.</p><p>The L3 (4 MB, 16-way) has 4096 logical set slots across four slices; this diagram labels the line by PA[17:6] as logical set <b>' + l3set + '</b>. This page does not model the real chip\u2019s address-to-slice mapping.</p><p>Beyond the L1, physical bits above bit 11 can affect set selection, so translation must complete before an L2 lookup.</p>');
  ...
  function mlpDraw(){
    ...
    mcap.innerHTML = '<p>Independent addresses are all known up front, so the core issues the four loads within a few cycles and the misses overlap (' + g('mlp') + '). Zen/Zen+ track 50 outstanding L2\u2192L3 misses per core and 96 L3\u2192memory, so the independent case scales far past four.</p><p>A pointer chase cannot overlap: each address is the previous load\u2019s data, so each miss starts only when the last one finishes. This is why latency benchmarks use a pointer chase: it exposes one full miss at a time.</p>';
  }
  mlpDraw();
  ...
  bus.innerHTML = '<h3>The wires between the levels</h3><table class="mt"><tr><th>link</th><th>width</th><th>one 64-byte line</th><th>source</th></tr>' +
    ...
    '<p class="note" style="margin-top:8px">Peak numbers only. Sustained bandwidth is lower: DRAM spends time on row switches and refresh ([[ch:dram]]).</p>';
