/* ======================= chapter: DRAM ======================= */
App.chapter({id: 'dram', short: 'DRAM', title: 'DRAM: banks, rows and the commands behind one read',
lede: 'The memory controller turns each 64-byte read into a short sequence of DRAM commands.',
...
build: function(root){
  ...
  h('div', {'class': 'card'}, intro, '<h3>The timings used here</h3><table class="mt"><tr><th>name</th><th>meaning</th><th>tCK</th><th>ns</th></tr>' +
    ...
    '</table><p class="note" style="margin-top:6px">A common DDR4-2400 CL17 speed bin (tCK = 0.833 ns), JEDEC-style values. Installed modules may differ: <code>sudo decode-dimms</code> or <code>sudo dmidecode -t memory</code> shows what is installed. Example: one rank of x8 chips per channel.</p>');
  ...
  var FR = [
    {p: 'map', t: -1, n: 0, sel: 'A', h: 'Physical address \u2192 DRAM coordinates', d: 'The controller slices each physical address into channel, bank group, bank, row and column. This page uses a simple example layout:\n<ul><li>bits 5:0: byte within the 64-byte burst</li><li>bit 6: channel</li><li>bits 13:7: column (64-byte units)</li><li>bits 15:14: bank</li><li>bits 17:16: bank group</li><li>bits 33:18: row</li></ul>\nFour reads are queued, all on channel 1:\n<ul><li><b>A</b>: C1\u2019s <code>hist[123]</code> line from [[ch:hier]] (PA 0x1a3f7ce40)</li><li><b>B</b>: another hist line in the <b>same row</b> as A</li><li><b>C</b>: 256 KB above A, so the same bank but a <b>different row</b></li><li><b>D</b>: the <code>data[]</code> line</li></ul>'},
    {p: 'ACT A', t: 0, n: 1, sel: 'A', h: 't = 0: ACT opens row 26877 in bank 3', d: 'Bank 3 is idle (precharged). ' + g('act', 'ACT') + ' drives one wordline: every cell on row 26877 shares its capacitor charge with its bitline.\nThe sense amplifiers turn each tiny voltage swing into a full 0 or 1 and latch it, so the whole row \u2014 1 KB per chip, 8 KB across the rank \u2014 now sits in the ' + g('rowbuf') + '. Reading drained the capacitors, so the sense amps also drive the values back into the cells.\nA column can be read only tRCD = 17 cycles (14.2 ns) later.'},
    ...
    {p: 'REF', t: 100, n: 8, sel: null, ref: true, h: 'Refresh', d: 'Cell capacitors leak. Every 7.8 \u00b5s (tREFI) the controller closes all banks of the rank and issues ' + g('refresh', 'REF') + '; for tRFC (350 ns for 8 Gb chips, JEDEC) the rank serves no reads.\n8192 REF commands every 64 ms cover every row.\nA read that arrives during tRFC waits for it: one source of the spread visible in a measured DRAM-latency plateau.'}
  ];
