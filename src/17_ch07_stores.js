...
App.chapter({id: 'stores', num: '07', short: 'Stores', title: 'The store path, memory types and fences',
sub: 'A store is invisible to everyone else until it leaves the store queue, and it can leave only when the L1d owns the line. What that costs depends on who else holds the line, on the memory type of the page, and on the fences around it.',
build: function(root){
  ...
  function frames(){
    ...
    F('execute', 'C1\u2019s store executes', 'The ' + g('sta') + ' writes address <code>0x7ffd4a3c2e58</code> and the ' + g('std') + ' writes 42 into C1\u2019s store-queue entry. No other core can see it. Younger loads on this core can, through ' + g('stlf') + ' (Chapter 02).', {sq: ['exec', 'none'], l0: l0, v0: 41, c1: c1, arr: []});
    ...
    F('stale', 'DRAM is now stale', 'DRAM still holds 41. It is updated only when the line is evicted and written back (Chapter 04), or when another core\u2019s request makes this core supply the data (Chapter 08). Until then the only current copy of hist[123] is in core 0\u2019s L1d.', {sq: ['done', 'done'], l0: 'M', v0: 43, c1: 'I', arr: [], stale: true});
  ...
  nt.innerHTML = '<h3>Filling a buffer: normal stores vs ' + g('nt', 'non-temporal stores') + '</h3><p>Normal stores to write-back memory must own each line first (RFO), so writing a buffer reads it from memory and later writes it back. Non-temporal stores (<code>movnti</code>, <code>movntdq</code>) gather bytes in ' + g('wc', 'write-combining') + ' buffers and send each completed 64-byte line straight to memory: no read, and the caches are not filled with data nobody will read soon. They are weakly ordered, so an <code>sfence</code> must come before any flag that publishes the buffer.</p>';
  ...
  mt.innerHTML = '<h3>' + g('memtype', 'Memory types') + '</h3><table class="mt"><tr><th>type</th><th>reads</th><th>writes</th><th>ordering / speculation</th><th>typical use</th></tr>' +
    ...
    '<tr><td>UC</td><td style="font-family:var(--sans)">not cached, each access sent</td><td style="font-family:var(--sans)">not cached, each access sent</td><td style="font-family:var(--sans)">strict program order; no speculative reads</td><td style="font-family:var(--sans)">device registers: the NVMe doorbell of Chapter 10</td></tr>' +
    ...
    '<p class="note" style="margin-top:6px">The type of an access comes from the ' + g('pat', 'PAT') + ' bits in the page-table entry (PAT, PCD, PWT: Chapter 03 showed PCD = PWT = 0 for hist, which selects WB) combined with the MTRRs that cover the physical range.</p>';
  ...
  h('div', {'class': 'card'}, ins, '<h3>Cache-control and ordering instructions</h3><table class="mt"><tr><th>instruction</th><th>effect</th></tr>' +
    ...
    .map(function(r){ return '<tr><td>' + r[0] + '</td><td style="font-family:var(--sans)">' + r[1] + '</td></tr>'; }).join('') + '</table><p class="note" style="margin-top:6px">Which of these your CPU implements: look for <code>clflushopt</code> and <code>clwb</code> in the flags line of <code>/proc/cpuinfo</code>.</p>');
